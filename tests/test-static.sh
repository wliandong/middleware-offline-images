...
test "$MYSQL_MIRROR_PREFIXES" = "docker.m.daocloud.io ghcr.io/wliandong docker.1ms.run dockerproxy.net"
test "$REDIS_MIRROR_PREFIXES" = "ghcr.io/wliandong"
test "$MONGODB_MIRROR_PREFIXES" = "ghcr.io/wliandong"
test "$KAFKA_MIRROR_PREFIXES" = "docker.m.daocloud.io ghcr.io/wliandong"
...
grep -F 'mirror_list_var="${key}_MIRROR_PREFIXES"' "$PROBE_SCRIPT"
grep -F 'No configured mirror provides a linux/amd64 image for' "$PROBE_SCRIPT"
...
WORKFLOW="$ROOT/.github/workflows/sync-images.yml"
test -f "$WORKFLOW"
grep -F 'skopeo copy --all' "$WORKFLOW"
grep -F 'docker.io/library/mysql:8.4.10' "$WORKFLOW"
grep -F 'docker.io/library/redis:8.8' "$WORKFLOW"
grep -F 'docker.io/library/mongo:8.0.26' "$WORKFLOW"
grep -F 'docker.io/apache/kafka:4.3.1' "$WORKFLOW"
grep -F 'ghcr.io/wliandong/library/mysql:8.4.10' "$WORKFLOW"
grep -F 'ghcr.io/wliandong/library/redis:8.8' "$WORKFLOW"
...
grep -F 'push:' "$OFFLINE_EXPORT_WORKFLOW"
grep -F '.github/workflows/export-offline-images.yml' "$OFFLINE_EXPORT_WORKFLOW"
