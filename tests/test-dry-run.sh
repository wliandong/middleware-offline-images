...
grep -F "MYSQL_MIRROR_PREFIXES='docker.m.daocloud.io ghcr.io/wliandong docker.1ms.run dockerproxy.net'" <<<"$probe_payload"
grep -F "REDIS_MIRROR_PREFIXES='ghcr.io/wliandong'" <<<"$probe_payload"
grep -F "MYSQL_IMAGE='library/mysql:8.4.10'" <<<"$probe_payload"
grep -F 'mirror_list_var="${key}_MIRROR_PREFIXES"' <<<"$probe_payload"
grep -F "MANIFEST_TIMEOUT_SECONDS='20'" <<<"$probe_payload"
grep -F 'timeout --signal=TERM --kill-after=2s 20s docker manifest inspect --verbose "$reference"' <<<"$probe_payload"
